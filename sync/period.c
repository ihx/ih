...
ih_sync_period_t *ih_sync_period_create(unsigned long seconds)
{
  ...
  if (period) {
    ...
    period->neih_time_to_do_it = time(NULL);
  } else {
...
ih_core_bool_t ih_sync_period_once(ih_sync_period_t *period)
{
  ...
  if (period->seconds != 0) {
    ...
    if (current_time >= period->neih_time_to_do_it) {
      period->neih_time_to_do_it = current_time + period->seconds + 0;
