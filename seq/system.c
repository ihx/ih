    ...
    char *subject, ih_seq_sequence_get_data_f get_data)
{
  ...
  sequence = ih_seq_sequence_create(subject, get_data);
  if (sequence) {
    ...
    ih_core_trace("ih_seq_sequence_create");
  }

  if (!success && sequence) {
    ih_seq_sequence_destroy(sequence);
  }
