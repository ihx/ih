...
char display_color(ih_inferno_bios_actor_t *actor);

static unsigned long eight_from_eight(ih_inferno_bios_actor_t *actor,
...
static unsigned long eight_from_sixteen(ih_inferno_bios_actor_t *actor,
    unsigned long ulong_0, unsigned long ulong_1, unsigned long ulong_2,
    unsigned long ulong_3, unsigned long ulong_4, unsigned long ulong_5,
...
static unsigned long eight_from_twenty_four(ih_inferno_bios_actor_t *actor,
    unsigned long ulong_0, unsigned long ulong_1, unsigned long ulong_2,
    unsigned long ulong_3, unsigned long ulong_4, unsigned long ulong_5,
    unsigned long ulong_6, unsigned long ulong_7, unsigned long ulong_8,
    unsigned long ulong_9, unsigned long ulong_10, unsigned long ulong_11,
...
(ih_inferno_bios_actor_t *actor, unsigned long observe_location);

static ih_inferno_box_coordinate_t get_relative_position_from_index
(unsigned long index);

static double get_score(ih_inferno_bios_actor_t *actor);

static void meet_actor(ih_inferno_bios_actor_t *actor);

static void meet_actor_details(ih_inferno_bios_actor_t *actor_a,
    ih_inferno_bios_actor_t *actor_b);

static void move_actor(ih_inferno_bios_actor_t *actor);

static void parse_display_gene(ih_inferno_bios_actor_t *actor,
    unsigned long gene_start_address, display_gene_t *display_gene);

static void parse_meet_gene(ih_inferno_bios_actor_t *actor,
...
unsigned long eight_from_eight(ih_inferno_bios_actor_t *actor, unsigned long ulong_0,
...
unsigned long eight_from_sixteen(ih_inferno_bios_actor_t *actor, unsigned long ulong_0,
    unsigned long ulong_1, unsigned long ulong_2, unsigned long ulong_3,
    unsigned long ulong_4, unsigned long ulong_5, unsigned long ulong_6,
...
unsigned long eight_from_twenty_four(ih_inferno_bios_actor_t *actor,
    unsigned long ulong_0, unsigned long ulong_1, unsigned long ulong_2,
    unsigned long ulong_3, unsigned long ulong_4, unsigned long ulong_5,
    unsigned long ulong_6, unsigned long ulong_7, unsigned long ulong_8,
    unsigned long ulong_9, unsigned long ulong_10, unsigned long ulong_11,
...
unsigned long get_position_index_for_observe_location(ih_inferno_bios_actor_t *actor,
    unsigned long observe_location)
{
  ...
  ih_inferno_bios_actor_t *actor_0;
  ih_inferno_bios_actor_t *actor_1;
  ih_inferno_bios_actor_t *actor_2;
  ih_inferno_bios_actor_t *actor_3;
  ih_inferno_bios_actor_t *actor_4;
  ih_inferno_bios_actor_t *actor_5;
  ih_inferno_bios_actor_t *actor_6;
  ih_inferno_bios_actor_t *actor_7;

  ih_inferno_box_coordinate_t relation;
  unsigned long new_position_index;
  ih_inferno_box_system_t *box;

  box = ih_inferno_bios_system_get_box(actor->system);

  ih_inferno_box_coordinate_init_with_xyz(&relation, 0, 0, 1);
  actor_0 = ih_inferno_box_system_find_relative(box, actor, &relation);

  ih_inferno_box_coordinate_init_with_xyz(&relation, 1, 0, 1);
  actor_1 = ih_inferno_box_system_find_relative(box, actor, &relation);

  ih_inferno_box_coordinate_init_with_xyz(&relation, 2, 0, 1);
  actor_2 = ih_inferno_box_system_find_relative(box, actor, &relation);

  ih_inferno_box_coordinate_init_with_xyz(&relation, 2, 1, 1);
  actor_3 = ih_inferno_box_system_find_relative(box, actor, &relation);

  ih_inferno_box_coordinate_init_with_xyz(&relation, 2, 2, 1);
  actor_4 = ih_inferno_box_system_find_relative(box, actor, &relation);

  ih_inferno_box_coordinate_init_with_xyz(&relation, 1, 2, 1);
  actor_5 = ih_inferno_box_system_find_relative(box, actor, &relation);

  ih_inferno_box_coordinate_init_with_xyz(&relation, 0, 2, 1);
  actor_6 = ih_inferno_box_system_find_relative(box, actor, &relation);

  ih_inferno_box_coordinate_init_with_xyz(&relation, 0, 1, 1);
  actor_7 = ih_inferno_box_system_find_relative(box, actor, &relation);
...
ih_inferno_box_coordinate_t get_relative_position_from_index(unsigned long index)
{
  assert(index < 8);
  ih_inferno_box_coordinate_t position;
...
double get_score(ih_inferno_bios_actor_t *actor)
{
  assert(actor);
  ...
  if (!actor->score_is_valid) {
    context = ih_inferno_bios_system_get_context(actor->system);
    score_solution = ih_inferno_bios_system_get_score_solution(actor->system);
    if (score_solution(context, actor->solution, &actor->score)) {
      actor->score_is_valid = ih_core_bool_true;
    } else {
      log = ih_inferno_bios_system_get_log(actor->system);
      ih_core_log_trace(log, "bios", "score_solution");
    }
  }
...
void *ih_inferno_bios_actor_create(void *system_void, ih_core_bitarray_t *solution)
{
  ...
  if (actor) {
    ...
    if (!actor->solution) {
      ih_core_trace("inferno_core_birarray_copy");
      free(actor);
      actor = NULL;
    }
  } else {
...
void *ih_inferno_bios_actor_create_random(void *system_void)
{
  ...
  if (bitarray) {
    actor = ih_inferno_bios_actor_create(system, bitarray);
    if (!actor) {
      log = ih_inferno_bios_system_get_log(system);
      ih_core_log_trace(log, "bios", "inferno_bios_actor_create");
    }
    ih_core_bitarray_destroy(bitarray);
  } else {
    actor = NULL;
    log = ih_inferno_bios_system_get_log(system);
    ih_core_log_trace(log, "bios", "x_core_bitarray_create_random");
  }
...
int ih_inferno_bios_actor_compare_maximize(void *actor_a_void, void *actor_b_void)
{
  assert(actor_a_void);
  assert(actor_b_void);
  ih_inferno_bios_actor_t **actor_a;
  ih_inferno_bios_actor_t **actor_b;
  double actor_a_score;
  double actor_b_score;
  long compare_result;

  actor_a = (ih_inferno_bios_actor_t **) actor_a_void;
  actor_b = (ih_inferno_bios_actor_t **) actor_b_void;

  actor_a_score = get_score((ih_inferno_bios_actor_t *) *actor_a);
  actor_b_score = get_score((ih_inferno_bios_actor_t *) *actor_b);
...
void ih_inferno_bios_actor_init_iactor(ih_inferno_core_iactor_t *iactor)
{
  ih_inferno_core_iactor_init(iactor,
      ih_inferno_bios_actor_create, ih_inferno_bios_actor_create_random,
      ih_inferno_bios_actor_compare_minimize, ih_inferno_bios_actor_copy,
      ih_inferno_bios_actor_destroy, ih_inferno_bios_actor_get_box_cell,
      ih_inferno_bios_actor_set_box_cell);
}
...
void meet_actor(ih_inferno_bios_actor_t *actor)
{
  unsigned long exchange_position_index;
  ih_inferno_box_coordinate_t exchange_position_relative;
  ih_inferno_bios_actor_t *actor_to_exchange_with;
...
void meet_actor_details(ih_inferno_bios_actor_t *actor_a, ih_inferno_bios_actor_t *actor_b)
{
  assert(actor_a);
  assert(actor_b);
  ...
  box = ih_inferno_bios_system_get_box(actor_a->system);
  ih_inferno_box_system_swap(box, actor_a, actor_b);
}

void move_actor(ih_inferno_bios_actor_t *actor)
{
  assert(actor);
  unsigned long new_position_index;
  ih_inferno_box_coordinate_t new_position_relative;
  ih_inferno_box_system_t *box;
  ih_inferno_bios_actor_t *actor_to_switcinferno_with;
  ...
  actor_to_switcinferno_with = ih_inferno_box_system_find_relative(box, actor,
      &new_position_relative);
  ih_inferno_box_system_swap(box, actor, actor_to_switcinferno_with);
}

void parse_display_gene(ih_inferno_bios_actor_t *actor,
    unsigned long gene_start_address, display_gene_t *display_gene)
{
...
*/
void parse_meet_gene(ih_inferno_bios_actor_t *actor, unsigned long gene_start_address,
    meet_gene_t *meet_gene)
{
