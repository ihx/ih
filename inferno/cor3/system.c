...
#include "ih/inferno/cor3/constants.h"
    ...
    ih_core_log_t *log)
{
  ...
  if (system) {
    ...
    ih_inferno_box_coordinate_init_with_xyz(&dimension_coordinate,
        IH_INFERNO_COR3_BOX_DIMENSION, IH_INFERNO_COR3_BOX_DIMENSION, IH_INFERNO_COR3_BOX_DIMENSION);
    ih_inferno_cor3_actor_init_iactor(&system->iactor);
    system->box = ih_inferno_core_create_actor_box(system, &dimension_coordinate,
        initial_solutions, &system->iactor, log);
    if (!system->box) {
      ih_core_log_trace(log, "cor3", "inferno_core_create_actor_box");
      free(system);
      system = NULL;
    }
  } else {
    ...
    unsigned short max_solution_count)
{
  ...
  solutions = ih_inferno_core_create_solutions_from_box(system->box,
      max_solution_count, ih_inferno_cor3_actor_get_solution, system->score_solution,
      system->goal, ih_inferno_cor3_actor_compare_maximize,
      ih_inferno_cor3_actor_compare_minimize, ih_inferno_cor3_actor_copy, system->context,
      system->log);
  if (!solutions) {
    ih_core_log_trace(system->log, "cor3", "create_solutions_from_box");
  }
...
{
  ih_inferno_search_isearch_init(isearch, ih_inferno_cor3_system_create,
      ih_inferno_cor3_system_destroy, ih_inferno_cor3_system_get_solutions_copy,
      ih_inferno_cor3_system_search);
}
...
void ih_inferno_cor3_system_search(void *system_object)
{
  ...
  ih_inferno_cor3_actor_act(actor);
}
