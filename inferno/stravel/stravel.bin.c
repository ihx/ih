...
#define DEFAULT_SEARCH_ALGORITHM IH_INFERNO_SEARCH_ALGORITHM_BIOS
...
static ih_core_bool_t save_points_to_file(stravel_t *stravel,
    char *filename);

static ih_core_bool_t save_solution_to_file(stravel_t *stravel,
    ih_core_bitarray_t *solution, char *filename);

static ih_core_bool_t score_solution(void *context,
    ...
    unsigned short point_b)
{
  ...
  d = ih_math_tools_calculate_distance(x1, y1, x2, y2);
...
ih_core_bool_t load_points_from_file(stravel_t *stravel,
    char *points_filename)
{
  ih_file_csv_t *csv;
  ...
  if (csv) {
    ...
    ih_core_log_enter(stravel->log, "stvl", "loading %lu points from %s",
    ...
    ih_core_log_trace(stravel->log, "stvl", "x_file_csv_create");
  }
...
ih_core_bool_t save_points_to_file(stravel_t *stravel, char *filename)
{
  ...
  if (file) {
    ...
    if (!ih_file_basic_write_string(file, "x,y\n")) {
      success = ih_core_bool_false;
      ih_core_log_trace(stravel->log, "stvl", "x_file_basic_write_string");
    }
    ...
    for (each_point = 0; each_point < stravel->point_count; each_point++) {
      ...
      if (sprintf(string, "%.5f,%.5f\n", x, y) >= 0) {  /*  TODO  */
        ...
        success = ih_core_bool_false;
        ih_core_log_trace(stravel->log, "stvl", "sprintf");
        break;
      }
    }
    ...
    success = ih_core_bool_false;
    ih_core_log_trace(stravel->log, "stvl", "x_file_basic_create");
  }
...
int main(int argc, char *argv[])
{
  ...
  ih_core_log_t *log;
  char *points_filename;
  ih_inferno_search_system_t *system;
  ih_case_array_t *initial_solutions;
  ...
  ih_inferno_search_algorithm_t search_algorithm;
  ...
  if (!options) {
    ih_core_log_trace_exit(log, "stvl", "x_config_options_create");
  }

  if (ih_config_options_find_as_string(options, "points_filename",
          &points_filename, "")) {
    if (!load_points_from_file(&stravel, points_filename)) {
      ih_core_log_trace_exit(log, "stvl", "load_points_from_file");
    }
  } else {
    ih_core_log_enter
  ...
  if (ih_config_options_find_as_string(options, "search_algorithm",
    ...
    if (!ih_inferno_search_algorithm_get_from_string
  ...
  search_algorithm_string
    = ih_inferno_search_algorithm_get_string(search_algorithm);
  ih_core_log_enter(log, "stvl", "search algorithm: %s",
      search_algorithm_string);

  ih_core_log_enter(log, "stvl", "saving points to stravel_points.csv");
  if (!save_points_to_file(&stravel, POINTS_FILENAME)) {
    ih_core_log_trace_exit(log, "stvl", "save_points_to_file");
  }

  sqrt_point_count = sqrt(stravel.point_count);
  goal_distance = ((POINT_RANGE * sqrt_point_count) + POINT_RANGE) * 1.0;

  ih_core_log_enter(log, "stvl", "caching point distances");
  cache_distances(&stravel);

  ih_core_log_enter(log, "stvl", "creating search system");
  system = ih_inferno_search_system_create(score_solution,
      IH_INFERNO_CORE_GOAL_MINIMIZE_SCORE, &stravel, initial_solutions,
      search_algorithm, log);
  if (system) {
    ih_core_log_enter(log, "stvl", "goal distance: %.2f", goal_distance);
    gettimeofday(&start_time, NULL);
    still_searching = ih_core_bool_true;
    while (still_searching) {
      ih_inferno_search_system_search(system, MAX_SEARCH_TIME_MICROSECONDS);
      solutions = ih_inferno_search_system_get_solutions_copy(system, 1);
      if (solutions) {
        solution = ih_case_array_find(solutions, 0);
        if (!score_solution(&stravel, solution, &best_score)) {
          ih_core_log_trace_exit(log, "stvl", "score_solution");
        }
        solution_count = ih_case_array_get_size(solutions);
        for (i = 0; i < solution_count; i++) {
          solution = ih_case_array_find(solutions, i);
          if (!score_solution(&stravel, solution, &score)) {
            ih_core_log_trace_exit(log, "stvl", "score_solution");
          }
          if (ih_core_bool_true) {
            ih_core_log_enter(log, "stvl", "distance: %.2f", score);
          }
          fflush(stdout);
        ...
        ih_case_array_destroy(solutions);
      } else {
        ih_core_log_trace_exit
    ...
    if (solutions) {
      ...
      if (!save_solution_to_file(&stravel, solution, SOLUTION_FILENAME)) {
        ih_core_log_trace_exit(log, "stvl", "save_solution_to_file");
      }
      ih_case_array_destroy(solutions);
    } else {
      ih_core_log_trace_exit
    ...
    ih_inferno_search_system_destroy(system);
  } else {
    ih_core_log_trace_exit(log, "stvl", "inferno_search_system_create");
  }
