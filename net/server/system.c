...
#include "ih/net/server/socket.h"
...
#define IH_NET_SERVER_SYSTEM_SLEEP_MICROSECONDS \
...
struct engine_container_t {
  ...
  ih_case_list_t *thread_inboxes[MAX_THREADS_PER_ENGINE];
...
struct ih_net_server_system_t {
  ...
  ih_case_set_t *client_posts;
  ih_net_exchange_t *client_exchange;
  pthread_mutex_t client_posts_mutex;

  ih_case_list_t *engines;
  engine_container_t *engines_array[IH_NET_SERVER_SYSTEM_MAX_ENGINES];
  ...
  ih_core_iobject_t client_posts_iobject;
};
...
static void close_disconnected_clients(ih_net_server_system_t *server);

static void close_unresponsive_clients(ih_net_server_system_t *server);

static engine_container_t *create_engine_container(ih_net_server_system_t *server,
    ih_core_engine_id_t engine_id, void *custom_server_object,
...
(engine_container_t *engine_container, ih_core_log_t *log);
...
(engine_container_t *engine_container, ih_core_imessage_t *messagey,
    ih_core_log_t *log);

static maintaining_engine_t *create_maintaining_engine
(ih_net_server_system_t *server, void *engine_object,
    ih_core_engine_id_t engine_id,
    ih_net_engine_iengine_maintain_f maintain);

static ih_core_bool_t create_post_for_new_client
(ih_net_server_system_t *server, int client_socket);

static running_engine_t *create_running_engine
(ih_net_server_system_t *server, void *engine_object,
    ih_core_engine_id_t engine_id, ih_net_engine_iengine_start_f start,
    ih_net_engine_iengine_run_f run, ih_net_engine_iengine_stop_f stop,
    unsigned short thread_index);

static ih_core_bool_t create_thread(ih_net_server_system_t *server,
    pthread_t *pthread_address, ih_core_thread_f thread_function,
    void *thread);

static void deliver_messages_to_engine(ih_net_server_system_t *server,
    engine_container_t *engine_container);

static unsigned long deliver_messages_to_engine_thread
(ih_net_server_system_t *server, engine_container_t *engine_container,
    unsigned short thread_index,
    unsigned long target_number_of_messages_to_deliver);

static void deliver_messages_to_engines(ih_net_server_system_t *server);
...
static unsigned long engine_count(ih_net_server_system_t *server);

static ih_core_bool_t engine_is_registered(ih_net_server_system_t *server,
    ih_core_engine_id_t engine_id);

static void engine_sleep(ih_net_server_system_t *server,
    ih_core_engine_id_t engine_id);

static void *find_client_post(ih_net_server_system_t *server,
    int client_socket);

static void handle_signal(int signal);

static ih_core_bool_t ih_net_server_system_create_client_exchange
(ih_net_server_system_t *server);

static ih_core_bool_t ih_net_server_system_create_client_posts
(ih_net_server_system_t *server);

static ih_core_bool_t ih_net_server_system_create_engines
(ih_net_server_system_t *server);

static ih_core_bool_t ih_net_server_system_create_outbox
(ih_net_server_system_t *server);

static void ih_net_server_system_create_rollback
(ih_net_server_system_t *server);

static ih_core_bool_t ih_net_server_system_register_engine_create
(engine_container_t *engine_container, ih_net_engine_iengine_create_f create,
    ih_net_server_system_t *server, void *custom_server_object);

static ih_core_bool_t join_thread(ih_net_server_system_t *server,
    pthread_t thread);

static void *maintain_engine(void *maintaining_engine_object);

static void manage_threads(ih_net_server_system_t *server);

static void post_messages_to_clients(ih_net_server_system_t *server);

static void receive_messages_from_client_post(ih_net_server_system_t *server,
    void *post_object);

static void receive_messages_from_clients(ih_net_server_system_t *server);

static void *run_engine(void *running_engine_object);

static ih_core_bool_t serversocket_bind_listen
(ih_net_server_system_t *server);

static void *server_thread(void *server_object);

static void stop(ih_net_server_system_t *server);

static ih_core_bool_t start(ih_net_server_system_t *server);

static ih_core_bool_t start_engine(ih_net_server_system_t *server,
    ih_core_engine_id_t engine_id);

static ih_core_bool_t start_engine_maintain_thread
(ih_net_server_system_t *server, ih_core_engine_id_t engine_id,
    engine_container_t *engine_container);

static ih_core_bool_t start_engine_run_thread
(ih_net_server_system_t *server, ih_core_engine_id_t engine_id,
    engine_container_t *engine_container, unsigned short thread_index);

static ih_core_bool_t threads_maxed_out(ih_net_server_system_t *server);
...
void *accept_thread(void *server_object)
{
  ...
  while (!ih_core_stop_requested) {
    client_socket = ih_net_server_socket_accept
...
void close_disconnected_clients(ih_net_server_system_t *server)
{
  assert(ih_case_set_get_size(server->client_posts)
  ...
  ih_case_set_iterate_start(client_posts);
  while ((client_post_object = ih_case_set_iterate_next(client_posts))) {
    if (server->ipost->socket_closed(client_post_object)) {
      client_socket = server->ipost->get_socket(client_post_object);
      ih_case_set_iterate_remove(client_posts);
...
void close_unresponsive_clients(ih_net_server_system_t *server)
{
  assert(ih_case_set_get_size(server->client_posts)
      == ih_net_exchange_get_post_count(server->client_exchange));
  void *client_post_object;
...
engine_container_t *create_engine_container(ih_net_server_system_t *server,
    ih_core_engine_id_t engine_id, void *custom_server_object,
    ih_net_engine_iengine_t *iengine, unsigned short min_run_threads,
    unsigned short max_run_threads, ih_net_maintain_t maintain_schedule,
    unsigned long message_type_count)
{
  ...
  if (so_far_so_good) {
    engine_container->inbox = ih_case_list_create
...
(engine_container_t *engine_container, ih_core_log_t *log)
{
  ...
  engine_container->performance_period
    = ih_sync_period_create(PERFORMANCE_PERIOD_SECONDS);
...
void create_engine_container_rollback(engine_container_t *engine_container)
{
  ...
  if (engine_container->performance_period) {
    ih_sync_period_destroy(engine_container->performance_period);
  }
    ...
    ih_core_log_t *log)
{
  ...
  for (each_thread = 0; each_thread < MAX_THREADS_PER_ENGINE; each_thread++) {
    ...
    *(engine_container->thread_inboxes + each_thread)
      = ih_case_list_create(IH_CORE_OBJECT_NO_COMPARE_F,
...
ih_core_bool_t create_post_for_new_client(ih_net_server_system_t *server,
    int client_socket)
{
  assert(server);
  ih_core_bool_t success;
  ...
  if (post_object) {
    ...
    {
      if (ih_case_set_add(server->client_posts, post_object)) {
        success = ih_core_bool_true;
...
running_engine_t *create_running_engine(ih_net_server_system_t *server,
    void *engine_object, ih_core_engine_id_t engine_id,
    ih_net_engine_iengine_start_f start, ih_net_engine_iengine_run_f run,
    ih_net_engine_iengine_stop_f stop, unsigned short thread_index)
{
...
ih_core_bool_t create_thread(ih_net_server_system_t *server,
    pthread_t *pthread_address, ih_core_thread_f thread_function,
    void *thread)
{
  ih_core_bool_t success;
...
*/
void deliver_messages_to_engine(ih_net_server_system_t *server,
    engine_container_t *engine_container)
{
  assert(server);
  assert(engine_container);
  ...
  if (ih_sync_period_once(engine_container->performance_period)) {
  ...
  if (engine_container->run_thread_count > 0) {
    inbox_size = ih_case_list_get_size(engine_container->inbox);
    ...
    unsigned long target_number_of_messages_to_deliver)
{
  ...
  if (0 == pthread_mutex_trylock(thread_inbox_mutex)) {
    for (each_message = 0; each_message < target_number_of_messages_to_deliver;
         each_message++) {
      if ((message = ih_case_list_find_first(engine_container->inbox))) {
        if (ih_case_list_add_last(thread_inbox, message)) {
...
void deliver_messages_to_engines(ih_net_server_system_t *server)
{
  assert(server);
  engine_container_t *engine_container;

  ih_case_list_iterate_start(server->engines);
  while ((engine_container = ih_case_list_iterate_next(server->engines))) {
...
unsigned long engine_count(ih_net_server_system_t *server)
{
  return ih_case_list_get_size(server->engines);
}

ih_core_bool_t engine_is_registered(ih_net_server_system_t *server,
    ih_core_engine_id_t engine_id)
{
...
void engine_sleep(ih_net_server_system_t *server,
    ih_core_engine_id_t engine_id)
{
  usleep(server->engines_array[engine_id]->sleep);
}

void *find_client_post(ih_net_server_system_t *server, int client_socket)
{
  void *client_post_object;
    ...
    ih_core_log_t *log)
{
  ...
  if (so_far_so_good) {
    so_far_so_good = ih_net_server_system_create_outbox(server);
  }

  if (so_far_so_good) {
    so_far_so_good = ih_net_server_system_create_client_posts(server);
  }

  if (so_far_so_good) {
    so_far_so_good = ih_net_server_system_create_client_exchange(server);
  }

  if (so_far_so_good) {
    so_far_so_good = ih_net_server_system_create_engines(server);
  }

  if (!so_far_so_good) {
    ih_net_server_system_create_rollback(server);
    server = NULL;
  }
...
ih_core_bool_t ih_net_server_system_create_client_exchange
(ih_net_server_system_t *server)
{
  assert(server);
  ih_core_bool_t success;

  server->client_exchange = ih_net_exchange_create(server->ipost);
...
ih_core_bool_t ih_net_server_system_create_client_posts
(ih_net_server_system_t *server)
{
  assert(server);
  ih_core_bool_t success;
...
ih_core_bool_t ih_net_server_system_create_engines
(ih_net_server_system_t *server)
{
  assert(server);
  ih_core_bool_t success;
  ...
  server->engines = ih_case_list_create(IH_CORE_OBJECT_NO_COMPARE_F,
      IH_CORE_OBJECT_NO_COPY_F, destroy_engine_container);
  if (server->engines) {
    for (each_engine_id = 0;
         each_engine_id < IH_NET_SERVER_SYSTEM_MAX_ENGINES;
         each_engine_id++) {
...
ih_core_bool_t ih_net_server_system_create_outbox
(ih_net_server_system_t *server)
{
  assert(server);
  ih_core_bool_t success;

  server->outbox = ih_case_list_create(IH_CORE_OBJECT_NO_COMPARE_F,
...
void ih_net_server_system_create_rollback(ih_net_server_system_t *server)
{
  if (server) {
    if (server->name) {
      free(server->name);
    ...
    if (server->engines) {
      ih_case_list_destroy(server->engines);
    }
    free(server);
  }
}
...
void ih_net_server_system_destroy(ih_net_server_system_t *server)
{
  ...
  ih_case_list_iterate_start(server->outbox);
  while ((message_object = ih_case_list_iterate_next(server->outbox))) {
...
ih_net_server_system_get_handler_for_message
    ...
    ih_net_server_stats_t *server_stats)
{
  ...
  server_stats->connected_client_count = ih_case_set_get_size
    (server->client_posts);
...
void ih_net_server_system_print_stats(ih_net_server_system_t *server)
{
  ...
  ih_net_server_system_get_stats(server, &stats);
    ...
    ih_core_engine_id_t engine_id, unsigned short thread_index)
{
  ...
  ih_case_list_iterate_start(inbox);
  while ((message_object = ih_case_list_iterate_next(inbox))) {
    ...
    if (handler) {
      ...
      switch (message_status) {
        case IH_NET_MESSAGE_STATUS_HANDLED:
          server->stats.engine_handled_message_count++;
          ih_case_list_iterate_remove(inbox);
          break;
...
(ih_net_server_system_t *server, ih_core_engine_id_t engine_id,
    void *custom_server_object, ih_net_engine_iengine_t *iengine,
    ...
    ih_net_maintain_t maintain_schedule, unsigned long message_type_count)
{
  ...
  if (success) {
    success = ih_net_server_system_register_engine_create(engine_container,
...
ih_core_bool_t ih_net_server_system_register_engine_create
(engine_container_t *engine_container, ih_net_engine_iengine_create_f create,
    ih_net_server_system_t *server, void *custom_server_object)
{
  assert(engine_container);
...
(ih_net_server_system_t *server, void *message_object)
{
  ...
  if (ih_case_list_add_last(server->outbox, message_object)) {
    success = ih_core_bool_true;
  } else {
    success = ih_core_bool_false;
    server->stats.server_send_message_failure_count++;
...
ih_core_bool_t join_thread(ih_net_server_system_t *server,
    pthread_t thread)
{
  ih_core_bool_t success;
...
void *maintain_engine(void *maintaining_engine_object)
{
  ...
  ih_net_server_system_t *server;
  engine_container_t *engine_container;
  ih_sync_period_t *maintenance_period;
  ...
  maintenance_period
    = ih_sync_period_create(engine_container->maintain_schedule);
  if (maintenance_period) {
    while (!engine_container->stop_requested) {
      if (ih_sync_period_once(maintenance_period)) {
...
void manage_threads(ih_net_server_system_t *server)
{
}

void post_messages_to_clients(ih_net_server_system_t *server)
{
  assert(server);
  void *message_object;
  void *client_post_object;
...
void receive_messages_from_client_post(ih_net_server_system_t *server,
    void *post_object)
{
  assert(server);
  assert(post_object);
  ...
  while ((message_object = server->ipost->receive_message(post_object))) {
    ...
    if (engine_is_registered(server, engine_id)) {
      ...
      ih_core_log_enter(server->log, "inet",
          ...
          ih_core_engine_get_name(engine_id));
      server->stats.discarded_message_for_unregistered_engine_count++;
      server->messagey->destroy(message_object);
    }
  }
...
void receive_messages_from_clients(ih_net_server_system_t *server)
{
  void *client_post_object;
  ih_case_set_t *client_posts;
...
ih_core_bool_t serversocket_bind_listen(ih_net_server_system_t *server)
{
  assert(server);
  ih_core_bool_t success;
  unsigned short port;
...
ih_core_bool_t start(ih_net_server_system_t *server)
{
  engine_container_t *engine_container;
  ih_core_engine_id_t engine_id;
  ih_core_bool_t success;
...
ih_core_bool_t start_engine(ih_net_server_system_t *server,
    ih_core_engine_id_t engine_id)
{
  ih_core_bool_t success;
  unsigned short each_thread;
...
ih_core_bool_t start_engine_maintain_thread(ih_net_server_system_t *server,
    ih_core_engine_id_t engine_id, engine_container_t *engine_container)
{
  ih_core_bool_t success;
...
ih_core_bool_t start_engine_run_thread(ih_net_server_system_t *server,
    ih_core_engine_id_t engine_id, engine_container_t *engine_container,
    unsigned short thread_index)
{
  ih_core_bool_t success;
...
void stop(ih_net_server_system_t *server)
{
  unsigned short each_thread;
  unsigned short run_thread_count;
  engine_container_t *engine_container;
  ...
  */
  ...
  ih_net_server_socket_destroy(server->socket);
...
ih_core_bool_t threads_maxed_out(ih_net_server_system_t *server)
{
  assert(server);
