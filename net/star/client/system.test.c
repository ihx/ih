...
int main(int argc, char *argv[])
{
  ...
  period = ih_sync_period_create(8);
  if (!period) {
    ih_core_trace_exit("x_core_period_create");
  }

  starclient_conf = ih_net_star_client_conf_create("x_net_starclient_test.conf");
  if (!starclient_conf) {
    ih_core_trace_exit("x_net_starclient_conf_create");
  }

  starclient = ih_net_star_client_system_create(starclient_conf->star_arm_ips,
  ...
  if (starclient) {
    if (ih_net_star_client_system_connect(starclient)) {
      printf("starclient connected to star\n");
      while (!ih_core_stop_requested) {
        ih_net_star_client_system_process_messages(starclient);

        /*
          TODO: move this to the starclient
        */
        if (!ih_net_star_client_system_star_available(starclient)) {
          if (ih_sync_period_once(period)) {
    ...
    ih_core_trace("x_net_starclient_create");
  }

  ih_net_star_client_system_destroy(starclient);
  ih_net_star_client_conf_destroy(starclient_conf);
  ih_sync_period_destroy(period);
