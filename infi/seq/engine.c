    ...
    void *message_object)
{
  ...
  ih_infi_seq_ping_t *ping;
  ih_infi_seq_pong_t *pong;
  ih_core_message_t *pong_message;
  ih_infi_seq_engine_t *engine;
  ih_net_server_system_t *ih_net_server;
  ...
  ping = ih_infi_seq_ping_create_from_message(message);
  if (ping) {
    ping_string = ih_infi_seq_ping_get_string(ping);
    if (ping_string) {
      pong = ih_infi_seq_pong_create(ping_string);
      if (pong) {
        pong_message = ih_infi_seq_message_create
          (client_socket, IH_INFI_SEQ_MESSAGE_PONG);
        if (pong_message) {
          if (ih_infi_seq_pong_add_to_message(pong, pong_message)) {
        ...
        ih_infi_seq_pong_destroy(pong);
      } else {
        ih_core_trace("x_seq_pong_create");
        message_status = IH_NET_MESSAGE_STATUS_CANT_HANDLE;
      }
      ih_infi_seq_ping_destroy(ping);
    } else {
    ...
    void *custom_server_object)
{
  ...
  ih_net_server_system_register_message_handler(ih_net_server,
      IH_CORE_ENGINE_SEQ, IH_INFI_SEQ_MESSAGE_REQUEST_GET_LAST_N,
      handle_get_last_n);
  ih_net_server_system_register_message_handler(ih_net_server,
      IH_CORE_ENGINE_SEQ, IH_INFI_SEQ_MESSAGE_REQUEST_GET_SINCE_TIMESTAMP,
...
ih_net_server_system_handle_message_f ih_infi_seq_engine_get_handler_for_message
